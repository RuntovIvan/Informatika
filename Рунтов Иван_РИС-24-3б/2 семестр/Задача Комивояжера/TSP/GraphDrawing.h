...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->label14->Text = L"Начальная вершина";
			...
			this->btn_TSP->Text = L"Решить задачу коммивояжера";
...
// отрисовка
	...
	private: System::Void print_TSP_route(String^& str, std::vector<int> route)
	{
		...
		str += "\nСтоимость: " + System::Convert::ToString(graph->get_route_weight(route));
	}
	...
	private: System::Void btn_TSP_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		{
			MessageBox::Show("Введите вершину.", "Ошибка");
		}
		else
		{
			int vertex;
			...
			{
				MessageBox::Show("Вершины нет в графе.", "Ошибка");
				txt_TSP->Text = "";
				return;
			}
			...
			for (int i = 0; i < graph->vertList.size(); i++)
				for (int j = 0; j < graph->vertList.size(); j++)
					...
					{
						MessageBox::Show("Не хватает ребер.", "Ошибка");
						txt_TSP->Text = "";
						return;
					}
			...
			MessageBox::Show(message, "Решение");
			txt_TSP->Text = "";
		}
	}
};
