...
public:
	...
	int Fl_adjMatrix[maxSize][maxSize]; // матрица смежности для алгоритма Флойда
	int shortestPathsMatrix[maxSize][maxSize]; // матрица длин путей
	int secondMatrix[maxSize][maxSize]; // матрица построения путей
	...
	void DFS(const int& vert_start, std::vector<bool>& visited_verts, std::vector<int>& route); // поиск в глубину
	void BFS(const int& vert_start, std::vector<bool>& visited_verts, std::vector<int>& route); // поиск в ширину

	bool AllVisited(std::vector<bool> visited_verts); // прроверка все ли посещены
	void FillLabels(int& start_vertex); // заполнение списка меток
